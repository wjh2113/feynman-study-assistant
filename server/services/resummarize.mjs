 ...
 * @param {{ mapOnly?: boolean }} [options] mapOnly skips re-embedding (for delete/rebuild map).
 */
export async function resummarizeProject(projectId, userId, onProgress = () => {}, options = {}) {
  ...
  try {
    ...
    let allChunks = [];
    let totalParents = 0;
    const prevSources = project.analysis?.sources || [];
    ...
    for (const [index, document] of documents.entries()) {
      ...
      if (mapOnly) {
        storedSources.push({
          ...
          summary: prev?.summary || source.summary,
          parseReport: source.parseReport,
          parsedPreview: source.parsedPreview,
          outline: prev?.outline || source.outline
        });
        continue;
      }
    ...
    await recordEvent(userId, projectId, "project_resummarized", {
      ...
      chunks: mapOnly ? Number(project.analysis?.retrieval?.chunks || 0) : allChunks.length,
      mapOnly
    });
    onProgress(100);
    return {
    ...
    const failed = await getProject(projectId, userId);
    if (failed) {
      await saveProject({
        ...failed,
        userId,
        analysis: {
          ...
          contentAnalysisError: error.message || "重新总结失败",
